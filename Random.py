...
async def attack(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id=chat_id, text=( 
      maa chud gyi bgmi ki  f"*⚔️ Attack Launched! ⚔️*\n"
      ip daalo bhenchod  f"*🎯 Target: {ip}:{port}*\n"
     time lga kutte   f"*🕒 Duration: {duration} seconds*\n"
      maa chodd di bgmi ki  f"*🔥 Let the battlefield ignite! 💥*"
    ), parse_mode='Markdown')
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*✅ Attack Completed! ✅Maa chud gyi sab ki ab kya hoga kutte*\n*Thank you for using our service!*", parse_mode='Markdown')
